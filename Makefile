...
.PHONY: help setup up down status logs smoke api-up ui-up ui-down restart-ui
...
help:
	...
	@echo "  make up         Sobe API (se parada) + Streamlit"
	@echo "  make down       Para API e Streamlit"
	@echo "  make restart-ui Reinicia só o Streamlit (API continua rodando)"
...
api-up: $(RUN_DIR)
	@curl -sf "http://127.0.0.1:$(API_PORT)/health" 2>/dev/null | grep -q '"status":"ok"' && { \
...
ui-up: _check-env $(RUN_DIR)
	...
	@echo "Streamlit ready: http://127.0.0.1:$(UI_PORT)"

restart-ui: ui-up

up: _check-env $(RUN_DIR)
	@$(MAKE) --no-print-directory api-up
	...
	@echo "  Logs:      make logs"
	@echo "  Stop:      make down  ·  Só UI: make restart-ui"

...
CONTEXTO:
- GLOBAL (distribuidora): linhas clínicas e preços vêm do CSV de compras (CUSTO_ENTRADA)
  ou da base de custo/estoque (CUSTOREAL / CUSTOULTENT por SKU).
...
8. PREÇO: custo Global e VL Médio Unimed são hints (sinais preco_ok, faixa ~0,25× a 4×).
   NÃO rejeite um match clínico correto só por divergência de preço — sanidade de preço
   e projeções financeiras são calculadas depois no relatório. Use no_match por preço
   apenas quando a divergência indicar claramente produto, dose, embalagem ou unidade
   diferente (evidência clínica, não só ratio numérico).
9. Não force match só por dimensões vagas (ex.: saco 60L) se o uso clínico difere.

SINAIS NOS CANDIDATOS (hints — decisão final é sua):
- hash_match: mesmo medication_hash_id canônico (L2) que a linha Global normalizada.
- fuzzy_alta: alta similaridade textual com a linha Global.
- preco_ok: VL Médio Unimed compatível com custo Global (faixa ~0,25× a 4×).
Use os sinais como apoio; confirme sempre equivalência clínica (PA, dose, forma, via, volume).
...
def build_user_prompt(item_text: str, candidates_text: str = "") -> str:
    ...
    return (
        f"{item_text}\n\n"
        "Escolha UM cod_item da lista de candidatos Unimed (com sinais) ou retorne no_match."
    )
